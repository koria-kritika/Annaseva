...
export const sendForgotOtp = async (req, res, next) => {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:400px;margin:auto;padding:24px;border:1px solid #eee">
          <h2 style="color:#FC8019;letter-spacing:2px;text-transform:uppercase">AnnaSeva</h2>
          <p style="color:#333">Your password reset OTP is:</p>
          <div style="font-size:36px;font-weight:900;letter-spacing:8px;color:#1A1A1A;margin:16px 0">${otp}</div>
          <p style="color:#888;font-size:12px">This OTP expires in <strong>10 minutes</strong>. Do not share it with anyone.</p>
        </div>
      `,
    });
